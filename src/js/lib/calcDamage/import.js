...
export function setupImportForm() {
    $(`#${ids.importButton}`).on('click touchend', (ev) => {
        ev.preventDefault();
        const importFilePath = document.getElementById(ids.importFilePath);
...
export function importFromURL() {
    ...
    if (risonStr == '')
        return;
    // パラメータ読み込み
    const json = rison.decode(risonStr);
    importData(json);
}
/**
 ...
 */
export function importFromJSONString(jsonStr) {
    // パラメータ読み込み
