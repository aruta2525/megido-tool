...
import { defaultSkills } from './interface/ISkillData';
import { getPhotonCorrection } from './util/MegidoUtil';
import { PhotonType, ReferralStatus, Attribute } from './enum/Megido';
 ...
 * @param megido 表示する
 */
function setupSacredTreasures(megido) {
 ...
 * @return
 */
function generateSacredTreasureOptionsHTMLString(styleType) {
 ...
 * @return
 */
function generatePersonalSacredTreasureOptionsHTMLString(megido) {
    const selectDummy = $('<select></select>');
    // 専用霊宝配列を取得
    const personalSacredTreasureList = getPersonalSacredTreasureList(megido.clockType, megido.no, megido.styleType);
 ...
 * @param styleType 表示する霊宝のスタイル
 */
function setupSacredTreasure(selectId, textId, html, sacredTreasureList, megido) {
 ...
 */
function getSelectedSacredTreasure(selectId) {
    const stSelect = $(`#${selectId}`);
    const index = stSelect.val();
    const st = SacredTreasureList[Number(index)];
    return st;
...
function calculateDamage() {
    ...
    let damage = (baseDamage - enemyDefense * enemyDefensiveBuff * enemyDefensiveDebuff * ignoreEnemyDefense) *
        photonCorrection *
        classCorrection *
        attributeBuff *
        species *
