...
function setupSynchronizeInputWithRange(inputId, rangeId) {
    const input = $(`#${inputId}`);
    const range = $(`#${rangeId}`);
    range.on('input', (ev) => {
...
function reverseLookup(beforeDamage) {
...
function setNextValue(rangeId, step) {
    const range = $(`#${rangeId}`);
...
function syncronizeRange(ev) {
    const target = ev.target;
}
//# sourceMappingURL=hBomb.js.map